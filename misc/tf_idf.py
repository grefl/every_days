...
class TextBlob:
    ...
    def count(self, word):
        return sum(1 for w in self.words if w.lower() == word.lower())
...
def tf(word, blob):
    return blob.count(word) / len(blob.words)
...
for i, blob in enumerate(bloblist):
    ...
    scores = {word: tfidf(word, blob, bloblist) for word in 'do you like python?'.split()}
